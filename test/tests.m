%% Tests %%             -- Antonio Carretero Sahuquillo --
...
imds = imageDatastore("imgs\","IncludeSubfolders",true,"LabelSource","foldernames");
    ...
    %las imagenes con el mismo nombre que tienen las carpetas que las
    %contienen.
...
[imdsTrain,imdsValidation] = splitEachLabel(imds,0.7,'randomized');
    ...
    %Con una proporción del 70% para imágenes de entrenamiento (imdsTrain) y el resto de validación.

    %El tercer argumento indica que las divida de manera aleatoria, es
    %decir, que en cada imds habrá tanto imágenes con pulgones como sin
    %pulgones pero estarán correctamente etiquetadas.

%% Carga y ajuste de una red preentrenada --> "alexnet"
net = alexnet; %Cargo la arquitectura de la red en la variable net.
    ...
    %interactuar con ella de forma gráfica, descomentar para verla:
        %analyzeNetwork(net)

    %La primera capa de la red, como se observa descomentando lo de arriba,
    %requiere imágenes de entrada de un tamaño de 227 x 227 x 3, donde 3 es el número
    %de canales de color.
...
inputSize = net.Layers(1).InputSize; %Cargamos estas dimensiones de entrada en la variable inputsize.
    %Para visualizar el valor, descomentar: 
        %inputsize
   

%Las últimas tres capas de la red están configuradas para etiquetar 1000 clases.
...
%realizar más aumentos de datos ni realizar operaciones adicionales:
...
%Entrenamiento de la red con la función tranNetwork():
netTransfer = trainNetwork(augimdsTrain,layers,options);
    ...
    %modificadas como lo mencinado anteriormente.
...
%Para clasificar las imágenes con la red entrenada utilizo la función calssify().

%Visualización de cuatro imágenes clasificadas con la red:
